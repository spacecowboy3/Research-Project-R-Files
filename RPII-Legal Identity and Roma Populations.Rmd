...
library(dplyr)
hhr <- read.csv("path_to_your_file.csv")
...
hh <- hh %>%
  mutate(
    HouseHead_Religiosity_R = case_when(
      HC1A %in% c(1, 2, 3, 4, 6) ~ 1,   # Recode to 1 if HC1A is 1, 2, 3, 4, or 6
      HC1A == 7 ~ 0,                    # Recode to 0 if HC1A is 7
      TRUE ~ NA_real_                   # Assign NA for any other values or missing data
    )
  ) %>%
...
wm <- wm %>%
  mutate(
    desire_for_lastbirth_r = case_when(
      CM17 == 1 & DB2 == 1 ~ 1,   # If CM17 (DB1) is YES and DB2 is YES, assign 1
      CM17 == 1 & DB2 == 2 ~ 0,   # If CM17 (DB1) is YES and DB2 is NO, assign 0
      CM17 == 2 ~ NA_real_,       # If CM17 (DB1) is NO, assign NA
      TRUE ~ NA_real_             # For any other cases, assign NA
    )
  ) %>%
  relocate(desire_for_lastbirth_r, .after = DB2)  # Relocate the new variable after DB2
...
#Sexual_Orientation_Dummy: Indicates if the respondent felt discriminated against based on sexual orientation.
#Age_Dummy: Indicates if the respondent felt discriminated against based on age.
#Religion_Belief_Dummy: Indicates if the respondent felt discriminated against based on religion or belief.
#Other_Reason_Dummy: Indicates if the respondent felt discriminated against for any other reason.
#Assigns 1 if the discrimination is reported (value 1).
#Assigns 0 if discrimination is not reported (value 2).
#Assigns NA for any other case (e.g., DK or missing values).
...
wm <- wm %>%
  ...
  relocate(level_of_education_mother_r, .after = WB6A)
...
wm <- wm %>%
  mutate(
    ...
    Mother_Health_Insurance = case_when(
      # If CM1 is 1 (mother) and WB18 is 1 (has health insurance), set to 1 (insured)
      CM1 == 1 & WB18 == 1 ~ 1,  
      # If CM1 is 1 (mother) and WB18 is 2 (no health insurance), set to 0 (not insured)
      CM1 == 1 & WB18 == 2 ~ 0,    
      # If CM1 is 2 (not a mother), set to NA (not applicable)
      CM1 == 2 ~ NA_real_,                      
      # For any other case, set to NA
      TRUE ~ NA_real_                           
    )
  ) %>%
...
# Calculate the total number of assistance programs received
# Classify the families based on the assistance count
# Relocate the new variable after the last column of interest

# Create the new variable for family economic status based on social assistance responses

hh <- hh %>%
  mutate(
    # Classify the families based on the responses to each assistance program
    family_economic_status = case_when(
      # Poor: Yes to 3 or more programs
      (ST3_1 == 1) + (ST3_2 == 1) + (ST3_3 == 1) + (ST3_4 == 1) + (ST3_5 == 1) + (ST3_6 == 1) >= 3 ~ 0,
      
      # Average: Yes to 1 or 2 programs
      (ST3_1 == 1) + (ST3_2 == 1) + (ST3_3 == 1) + (ST3_4 == 1) + (ST3_5 == 1) + (ST3_6 == 1) >= 1 &
      (ST3_1 == 1) + (ST3_2 == 1) + (ST3_3 == 1) + (ST3_4 == 1) + (ST3_5 == 1) + (ST3_6 == 1) <= 2 ~ 1,
      
      # Rich: No to all programs
      (ST3_1 == 2) & (ST3_2 == 2) & (ST3_3 == 2) & (ST3_4 == 2) & (ST3_5 == 2) & (ST3_6 == 2) ~ 2,
      
      # Any other case should be treated as NA
      TRUE ~ NA_real_
    )
  ) %>%
  # Relocate the new variable after the last ST3 column
  relocate(family_economic_status, .after = last_col())
